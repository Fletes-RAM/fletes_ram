...
class InventarioMaterialSalidaController extends \BaseController {
	...
	{
		$this->layout           = View::make('sistema.inventariomaterial.createSalida');
		$this->layout->title    = 'Inventario Materiales Salida';
    ...
    $this->layout->breadcrumb = array(
      ...
      array(
				'title' => 'Inventario Materiales Salida',
				'link'  => 'material',
				'icon'  => 'fas fa-pallet'
      ),
    );
	...
	public function store()
	{
		...
		$materialSalida = new InventarioMaterialSalida();

		if (!$materialSalida->validate($new)) {
			$errors = $materialSalida->errors();
			return Redirect::route('materialSalida.create')->withInput()->withErrors($errors);
		}

		$materialSalida->create($new);
